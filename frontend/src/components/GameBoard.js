...
function GameBoard({ board, selectedTiles, setSelectedTiles }) {
  ...
  const isTripleWord = (row, col) => {
    const positions = [[0,0],[0,7],[0,14],[7,0],[7,14],[14,0],[14,7],[14,14]];
    return positions.some(([r, c]) => r === row && c === col);
  };

  const isDoubleWord = (row, col) => {
    const positions = [[1,1],[2,2],[3,3],[4,4],[1,13],[2,12],[3,11],[4,10],
                      [13,1],[12,2],[11,3],[10,4],[13,13],[12,12],[11,11],[10,10]];
    return positions.some(([r, c]) => r === row && c === col);
  };

  const isTripleLetter = (row, col) => {
    const positions = [[1,5],[1,9],[5,1],[5,5],[5,9],[5,13],[9,1],[9,5],
                      [9,9],[9,13],[13,5],[13,9]];
    return positions.some(([r, c]) => r === row && c === col);
  };

  const isDoubleLetter = (row, col) => {
    const positions = [[0,3],[0,11],[2,6],[2,8],[3,0],[3,7],[3,14],[6,2],
